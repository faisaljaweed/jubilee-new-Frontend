...
const megaMenuData: MegaMenuCategory[] = [
  ...
  },
  {
    ...
    icon: Heart,
    items: [
      { label: "ParentsCare Plus", href: "/health/parents-care-plus" },
      { label: "Family HealthCare", href: "/health/family-health-care" },
      { label: "Personal HealthCare", href: "/health/personal-health-care" },
      { label: "LifestyleCare", href: "/health/lifestyle-care" },
      { label: "ParentsCare", href: "/health/parents-care" },
      { label: "HerCare", href: "/health/her-care" },
      // { label: "OPD Coverage", href: "/healthcare/opd" },
      // { label: "Maternity Coverage", href: "/healthcare/maternity" },
      // { label: "Health Takaful", href: "/healthcare/takaful" },
      ...
      <header className="site-header absolute top-0 left-0 right-0 z-50">
        ...
        <div className="hidden lg:block">
          <div className="relative">
            ...
            <div
              className={`w-full ${
                pathname === "/" ? "bg-transparent" : "bg-white shadow-sm"
              }`}
            >
              <div className={`max-w-7xl mx-auto ${LOGO_PL} ${LOGO_PL_XL}`}>
                <div className="flex items-center justify-between px-5 py-3">
                      ...
                      <React.Fragment key={`${link.label}-${i}`}>
                        ...
                        {link.label === "ABOUT US" && (
                          ...
                          >
                            ...
                            {isProductsHovered && (
                              <div
                                className={`dropdown-enter absolute top-[calc(100%-2px)] left-[-120px] pt-3 transition-all duration-300 ${
                                  activeMegaMenu === "health"
                                    ? "w-[500px]"
                                    : "w-[230px]"
                                }`}
                                // onMouseEnter={handleProductsMouseEnter}
                              ...
                              >
                                <div className="rounded-b-[22px] bg-white shadow-2xl border border-gray-100 overflow-hidden transition-all duration-300">
                                  <div
                                    className={`grid gap-5 p-5 transition-all duration-300 ${
                                      activeMegaMenu === "health"
                                        ? "grid-cols-[190px_1fr]"
                                        : "grid-cols-[190px]"
                                    }`}
                                  >
                                      ...
                                      {megaMenuData.map((category) => {
                                        ...
                                        return (
                                          <Link
                                            ...
                                            className="flex items-center gap-2 rounded-[8px] bg-gray-100 px-4 py-2 text-left font-futura text-[15px] font-semibold text-[#202B4A] transition-all hover:bg-[#BA0C2F] hover:text-white"
                                          >
                                            <Icon size={20} />
                                            <span>{category.label}</span>
                                          </Link>
                                    ...
                                    {/* RIGHT HEALTH LINKS ONLY */}
                                    {activeMegaMenu === "health" &&
                                      healthCategory && (
                                        <div className="flex flex-col pr-2">
                                          {healthCategory.items.map(
                                            (item, index) => (
                                              <Link
                                                key={`health-${item.label}-${index}`}
                                                href={item.href}
                                                className="border-b border-gray-200 py-3 font-futura text-[17px] font-medium leading-[1.35] text-[#202B4A] transition hover:text-[#BA0C2F]"
                                                onClick={() => {
                                                  setIsProductsHovered(false);
                ...
                <div className="border-t border-gray-100 mt-1 pt-1">
                  ...
                  {isMobileProductsOpen && (
                    <div className="mt-2 flex flex-col gap-3 pb-2">
                      {megaMenuData.map((category) => {
                        const Icon = category.icon;
                        const isHealth = category.key === "health";
                        const isActive =
                          activeMegaMenu === "health" && isHealth;
                        const categoryHref = category.items[0]?.href || "#";
                        ...
                        if (!isHealth) {
                          return (
                            <Link
                              key={category.key}
                              href={categoryHref}
                              className="flex w-full items-center gap-2 rounded-xl bg-gray-100 px-4 py-3 font-futura text-sm font-semibold uppercase text-[#202B4A] transition hover:text-[#BA0C2F]"
                              onClick={() => {
                                setIsMenuOpen(false);
                                setIsMobileProductsOpen(false);
                                setActiveMegaMenu(null);
                        ...
                        return (
                          <div key={category.key}>
                            <button
                              type="button"
                              onClick={() =>
                                setActiveMegaMenu(isActive ? null : "health")
                              }
                              className={`flex w-full items-center justify-between rounded-xl px-4 py-3 font-futura text-sm font-semibold uppercase transition ${
                                isActive
                                  ? "bg-[#BA0C2F] text-white"
